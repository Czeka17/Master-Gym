...
function ContactForm(){
  const [formData, setFormData] = useState<FormData>({
    ...
    return <section className={styles['contact-us']} id="contact">
      {modalIsOpen && <FormModal onClose={hideModalHandler} title={title} message={message} />}
        <Title title='Kontakt'> 
        ...
        <div className={styles["contact-us__line"]}></div>
            <form onSubmit={handleSubmit} className={styles["contact-us__form"]}>
                ...
                <div className={styles["contact-us__form-box"]}>
                    <textarea name="message" id="message" required cols={30} rows={10} value={formData.message} onChange={handleChange} />
                    <span>Wiadomość</span>
                </div>
